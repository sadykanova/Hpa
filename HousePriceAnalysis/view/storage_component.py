...
def storage_component(master, app_size, data):
    ...
    while i <= 40*data_amount:
        ...
        tk.Button(master=frame, text='Delete', cursor='hand2',
                  command=lambda temp=j: storage.delete_data(data[temp]),
    ...
    if data:
        area_average = sum([int(i['price'][:-2]) for i in data]) / len(data)
        price_average = round(sum([float(i['area'][:-2].strip()) for i in data]) / len(data), 2)

        tk.Label(master=frame, text='一'*40, fg='black', bg='snow', font=('Times', 14, 'bold')).place(
            relx=0, y=i+10, relwidth=1, height=10)
        tk.Label(master=frame, text=f'A total of {data_amount} data.\n Average area {area_average}, average price {price_average}', fg='teal', bg='ghostwhite', font=('Times', 14, 'bold')).place(
            relx=0, y=i+35, relwidth=1, height=60)
    else:
